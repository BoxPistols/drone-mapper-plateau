...
```
Vite 8 · React 19 · TypeScript 5 · CesiumJS 1.139 · Zustand 5 · Anthropic Claude API
```
...
| **AIアシスタント** | 自然言語で飛行計画を生成（Claude Opus 4.6） |
...
# Anthropic API キー（AIアシスタント機能に必要）
...
### 3. Anthropic Claude API（AIアシスタント）
...
Claude API の **Tool Use（関数呼び出し）** 機能を使い、AIが直接アプリの状態を操作します：
...
const tools = [
  ...
  },
  {
    ...
    input_schema: { planId: string, waypoints: WaypointInput[] }
  },
  {
    name: 'select_city',
...
// src/ai/useAIChat.ts
// 1. ユーザーメッセージを送信
// 2. stop_reason === 'tool_use' になるまでストリーミング受信
// 3. ツール呼び出しがあればアプリ操作を実行
...
現在の実装はデモ目的でブラウザから直接 Anthropic API を呼び出しています（`dangerouslyAllowBrowser: true`）。本番環境では、APIキーをサーバーサイドに移動し、プロキシ経由でリクエストする必要があります。
...
src/
├── ai/
│   ├── tools.ts          # Claude AIツール定義・実行マッピング
...
| Zustand | 5 | グローバル状態管理 |
| `@anthropic-ai/sdk` | ^0.80 | Claude API クライアント |
