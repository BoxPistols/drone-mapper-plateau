...
// Catmull-Rom 接線ベクトル（heading・ピッチ算出用）
export function catmullRomTangent(p0: SplinePoint, p1: SplinePoint, p2: SplinePoint, p3: SplinePoint, t: number): { lon: number; lat: number; altAGL: number } {
  const t2 = t * t
  return {
    ...
    altAGL: 0.5 * ((-p0.altAGL + p2.altAGL) + (4 * p0.altAGL - 10 * p1.altAGL + 8 * p2.altAGL - 2 * p3.altAGL) * t + (-3 * p0.altAGL + 9 * p1.altAGL - 9 * p2.altAGL + 3 * p3.altAGL) * t2),
  }
}
